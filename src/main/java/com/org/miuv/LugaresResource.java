package com.org.miuv;


import com.org.models.Lugar;
import java.util.List;
import javax.ws.rs.core.Context;
import javax.ws.rs.core.UriInfo;
import javax.ws.rs.Consumes;
import javax.ws.rs.DELETE;
import javax.ws.rs.Produces;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.Path;
import javax.ws.rs.PUT;
import javax.ws.rs.PathParam;
import javax.ws.rs.core.MediaType;

/**
 * REST Web Service
 *
 * @author alsorc
 */
@Path("lugares")
public class LugaresResource {
    private IDao daoLugar;
    private Lugar lugar;
    private final String succesMessage = "OK";
    private final String errorMessage = "FAIL";

    @Context
    private UriInfo context;

    /**
     * Creates a new instance of LugaresResource
     */
    public LugaresResource() {
        daoLugar = new DaoLugares();
        lugar = new Lugar();
    }

   @GET
    @Produces(MediaType.APPLICATION_JSON)
    public List<Lugar> getAll() {
         return daoLugar.getRecords();
    }
    @GET
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{id}")
    public Lugar getOneByID(@PathParam("id")int id) {
        lugar.setIdLugar(id);
        return (Lugar)daoLugar.getOneRecord(lugar);
        
    }

    /**
     * PUT method for updating or creating an instance of CarrerasResource
     * @param content representation for the resource
     */
    @PUT
    @Consumes(MediaType.APPLICATION_JSON)
    public String update(Lugar lugar) {
        if(daoLugar.updateRecord(lugar))
            return succesMessage;
        else
            return errorMessage;
        
    }
@POST
@Consumes(MediaType.APPLICATION_JSON)
public String insert(Lugar lugar){
        if (daoLugar.insertRecord(lugar))
            return succesMessage;
        else
            return errorMessage;
}
@DELETE
@Consumes(MediaType.APPLICATION_JSON)
@Path("{id}")
    public String deleteOne(@PathParam("id") int id){
        lugar.setIdLugar(id);
        if(daoLugar.deleteRecord(lugar))
            return succesMessage;
        else
            return errorMessage;
    }
}
